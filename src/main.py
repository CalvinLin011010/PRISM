...
def main():
    ...
    if not os.path.exists(total_file):
        with open(total_file, 'w') as f:
            f.write("cur_time,data_name,model_type,HR@5,HR@10,HR@20,NDCG@5,NDCG@10,NDCG@20,use_sa,alpha_patch,seed,alpha,attn_drop,hidden_layer_drop,model_parameters,output_dir,args_namespace\n")

    model_parameters = sum([p.nelement() for p in model.parameters()])
    args_str = str(args).replace('\n', ' ').replace('\t', ' ')
    with open(total_file, 'a') as f:
        f.write(f"{cur_time},{args.data_name},{args.model_type},{result_info['HR@5']},{result_info['HR@10']},{result_info['HR@20']},{result_info['NDCG@5']},{result_info['NDCG@10']},{result_info['NDCG@20']},{str(args.use_sa)},{str(args.alpha_patch)},{str(args.seed)},{str(args.alpha)},{str(args.attention_probs_dropout_prob)},{str(args.hidden_dropout_prob)},{str(model_parameters)},{args.output_dir},{args_str}\n")
